#!/bin/bash
# fix_menu_permissions.sh - Correction des menus basés sur les permissions

set -e

echo "🔧 Correction des Menus Basés sur les Permissions"
echo "=============================================="

cd ~/e-FinTrack
source venv/bin/activate

echo "🔍 1. Configuration souhaitée:"
...
echo "🎯 Basé sur les permissions (peut_ajouter_xxx)"

echo ""
echo "🔧 2. Création du template basé sur les permissions..."

cat > templates/base_permissions.html << 'BASE_PERMISSIONS'
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
...
<body>
        ...
        <div class="row">
            ...
            <div class="col-md-2 sidebar">
                <nav class="nav flex-column">
                    <!-- Menu pour AdminDaf (uniquement natures) -->
                    {% if user.username == 'AdminDaf' %}
                    <div class="menu-section">
                        <div class="menu-title">Menu AdminDaf</div>
                    ...
                    <!-- Menu pour DirDaf et DivDaf (uniquement tableau de bord et clôtures) -->
                    {% if user.role == 'DG' or user.role == 'CD_FINANCE' %}
                    <div class="menu-section">
                        <div class="menu-title">Menu Direction</div>
                    ...
                    <!-- Menu pour OpsDaf (recettes, dépenses et états) -->
                    {% if user.role == 'OPERATEUR_SAISIE' %}
                    <div class="menu-section">
                        <div class="menu-title">Menu Opérations</div>
                        ...
                        <a class="nav-link" href="{% url 'tableau_bord_feuilles:etats_depenses' %}">
                            <i class="bi bi-file-earmark-text"></i> État Dépense
                        </a>
                        <a class="nav-link" href="{% url 'tableau_bord_feuilles:etats_recettes' %}">
                            <i class="bi bi-cash-coin"></i> État Recette
                        </a>
                        {% endif %}
                    </div>
                    {% endif %}
                    
                    <!-- Menu pour SuperAdmin (tout sauf natures) -->
                    {% if user.username == 'SuperAdmin' or user.is_superuser %}
                    <div class="menu-section">
                        <div class="menu-title">Menu SuperAdmin</div>
                        {% if user.peut_voir_tableau_bord %}
                        <a class="nav-link" href="{% url 'tableau_bord_feuilles:tableau_bord_feuilles' %}">
                            <i class="bi bi-speedometer2"></i> Tableau de bord
                        </a>
    ...
    <!-- Script de redirection basé sur les permissions -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var isSuperuser = "{{ user.is_superuser }}" === "True";
            
            // Permissions depuis le template
            var peutVoirTableauBord = "{{ user.peut_voir_tableau_bord }}" === "True";
            var peutAjouterNature = "{{ user.peut_ajouter_nature_economique }}" === "True";
            var peutCloturer = "{{ user.peut_cloturer }}" === "True";
            var peutAjouterRecetteDepense = "{{ user.peut_ajouter_recette_depense }}" === "True";
            var peutGenererEtats = "{{ user.peut_generer_etats }}" === "True";
            
            console.log("Menu permissions check:", {
                path: currentPath,
                username: username,
                userRole: userRole,
                isSuperuser: isSuperuser,
                peutVoirTableauBord: peutVoirTableauBord,
                peutAjouterNature: peutAjouterNature,
                peutCloturer: peutCloturer,
                peutAjouterRecetteDepense: peutAjouterRecetteDepense,
                peutGenererEtats: peutGenererEtats
            });
            ...
            if ((username === 'SuperAdmin' || isSuperuser) && currentPath === '/demandes/natures/') {
                console.log("Redirection SuperAdmin loin des natures");
                sessionStorage.setItem('redirected', 'true');
                window.location.href = '/tableau-bord-feuilles/';
                return;
            }
            
            // DirDaf et DivDaf : tableau de bord
            if ((userRole === 'DG' || userRole === 'CD_FINANCE') && currentPath === '/') {
                console.log("Redirection DirDaf/DivDaf vers tableau de bord");
                sessionStorage.setItem('redirected', 'true');
                window.location.href = '/tableau-bord-feuilles/';
                return;
            }
            
            // OpsDaf : tableau de bord
            if (userRole === 'OPERATEUR_SAISIE' && currentPath === '/') {
                console.log("Redirection OpsDaf vers tableau de bord");
                sessionStorage.setItem('redirected', 'true');
                window.location.href = '/tableau-bord-feuilles/';
                return;
            }
            
            // Réinitialiser le flag de redirection après 5 secondes
            setTimeout(function() {
...
echo "✅ Template base_permissions.html créé"

echo ""
echo "🔧 3. Remplacement du template..."

# Sauvegarder l'ancien template
cp templates/base.html templates/base_backup_permissions_$(date +%Y%m%d_%H%M%S).html

# Remplacer par la version basée sur les permissions
cp templates/base_permissions.html templates/base.html

echo "✅ Template base.html remplacé"

echo ""
echo "🌐 4. Test des menus basés sur les permissions..."

python manage.py shell << 'EOF'
from django.test import Client
from accounts.models import User

print("🧪 Test des menus basés sur les permissions:")
print("=" * 60)
...
try:
    ...
    print(f"   Permissions: peut_voir_tableau_bord={dir_daf.peut_voir_tableau_bord}, peut_cloturer={dir_daf.peut_cloturer}")
...
echo ""
echo "✅ Correction des menus basés sur les permissions terminée !"
echo ""
echo "🎯 Configuration basée sur les permissions:"
echo "📋 SuperAdmin → Voit tout SAUF les natures économiques"
echo "📋 DirDaf, DivDaf → Uniquement tableau de bord et clôtures"
echo "📋 AdminDaf → Uniquement natures économiques"
echo "📋 OpsDaf → Recettes, dépenses et états"
